...
write("app/diagnostics.py", '''from __future__ import annotations
...
routes = read("app/web/routes.py")
routes = replace_once(routes, "from app.serialization import json_safe\n", "from app.serialization import finite_number, json_safe, json_safe_dict\n", "routes serialization import")
routes = replace_once(
    routes,
...
write("app/web/routes.py", routes)

websocket = read("app/web/websocket.py")
...
''',
    "websocket source budget",
)
write("app/web/websocket.py", websocket)

# Database text/JSON boundaries never invoke hostile repr/str and queries are explicitly bounded.
database = read("app/storage/database.py")
...
database = replace_once(database, 'str(event.get("kind") or "unknown")[:120],', '_safe_text(event.get("kind"), "unknown", 120) or "unknown",', "database event kind")
database = replace_once(database, 'str(event.get("message") or "")[:4000],', '_safe_text(event.get("message"), "", 4000),', "database event message")
database = replace_once(database, '"data": json.loads(row["data_json"]),', '"data": _safe_json_loads(row["data_json"], {}),', "database event JSON")
database = replace_once(database, 'return json.loads(row["value"]) if row else None', 'return _safe_json_loads(row["value"], None) if row else None', "database metadata JSON")
database = replace_once(database, 'return json.loads(row["state_json"]) if row else None', 'return _safe_json_loads(row["state_json"], None) if row else None', "database snapshot JSON")
database = replace_once(
    database,
...
''',
    "database snapshots bound",
)
database = replace_once(database, '"response": json.loads(row["response_json"]),', '"response": _safe_json_loads(row["response_json"], {}),', "database response JSON")
database = replace_once(database, '"provider": json.loads(row["metadata_json"] or "{}"),', '"provider": _safe_json_loads(row["metadata_json"], {}),', "database provider JSON")
write("app/storage/database.py", database)

print("post5 phase5 applied")
